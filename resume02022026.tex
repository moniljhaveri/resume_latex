...
\begin{itemize}
	...
	\item \textit{Designed and created camera-service in python and a C++ dll to interface with vendor camera}
	\item \textit{Integrated datadog library into python service to collect metrics}
		\item \textit{Created python automation scripts to test camera-service using FastAPI and gRPC}
\end{itemize}
...
\begin{itemize}
	...
	\item \textit{Planned and implemented python model automation which reduced operational workload}	\item \textit{Modified alexa's  java client to work with multiple wake words on over 1 million devices}
\end{itemize}
